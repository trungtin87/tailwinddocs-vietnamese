...
export const description = "Các utility để kiểm soát vị trí đặt của các phần tử được định vị.";
...
Sử dụng các utility `top-<number>`, `right-<number>`, `bottom-<number>`, `left-<number>`, và `inset-<number>` như `top-0` và `bottom-4` để thiết lập vị trí ngang hoặc dọc của một [phần tử được định vị](/docs/position):
...
Sử dụng các utility `start-<number>` hoặc `end-<number>` như `start-0` và `end-4` để thiết lập các [thuộc tính logic](https://developer.mozilla.org/en-US/docs/Web/CSS/CSS_Logical_Properties/Basic_concepts) `inset-inline-start` và `inset-inline-end`, ánh xạ tới bên trái hoặc phải dựa trên hướng văn bản:
