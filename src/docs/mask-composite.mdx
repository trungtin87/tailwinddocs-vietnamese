...
export const description = "Các tiện ích để kiểm soát cách nhiều mask được kết hợp với nhau.";
...
Sử dụng các tiện ích như `mask-add` và `mask-intersect` để kiểm soát cách các mask của một phần tử được kết hợp với nhau:
