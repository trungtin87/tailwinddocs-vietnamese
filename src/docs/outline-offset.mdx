...
export const description = "Các tiện ích để kiểm soát offset của outline của một phần tử.";
...
Sử dụng các tiện ích như `outline-offset-2` và `outline-offset-4` để thay đổi offset của outline của một phần tử:
