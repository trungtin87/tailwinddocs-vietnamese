...
export const description = "Các tiện ích để kiểm soát padding của một phần tử.";
...
Sử dụng các tiện ích `pt-<number>`, `pr-<number>`, `pb-<number>`, và `pl-<number>` như `pt-6` và `pr-4` để kiểm soát padding ở một phía của một phần tử:
...
Sử dụng các tiện ích `px-<number>` như `px-4` và `px-8` để kiểm soát padding theo chiều ngang của một phần tử:
...
Sử dụng các tiện ích `py-<number>` như `py-4` và `py-8` để kiểm soát padding theo chiều dọc của một phần tử:
...
Sử dụng các tiện ích `ps-<number>` hoặc `pe-<number>` như `ps-4` và `pe-8` để đặt các thuộc tính logic `padding-inline-start` và `padding-inline-end`, ánh xạ tới phía bên trái hoặc bên phải dựa trên hướng văn bản:
...
Để kiểm soát nhiều hơn, bạn cũng có thể sử dụng [LTR và RTL modifiers](/docs/hover-focus-and-other-states#rtl-support) để áp dụng các kiểu cụ thể tùy thuộc vào hướng văn bản hiện tại.
