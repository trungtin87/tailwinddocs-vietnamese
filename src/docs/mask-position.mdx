...
export const description = "Các tiện ích để kiểm soát vị trí của mask image của một phần tử.";
...
Sử dụng các tiện ích như `mask-center`, `mask-right`, và `mask-left-top` để kiểm soát vị trí của mask image của một phần tử:
