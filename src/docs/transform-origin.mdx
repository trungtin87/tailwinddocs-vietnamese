...
export const description = "Các tiện ích để chỉ định điểm gốc (origin) cho các phép biến đổi của một phần tử.";
...
Sử dụng các tiện ích như `origin-top` và `origin-bottom-left` để đặt transform origin của một phần tử:
