...
export const description = "Các tiện ích để kiểm soát độ dày của text decoration.";
...
Sử dụng các tiện ích `decoration-<number>` như `decoration-2` và `decoration-4` để thay đổi độ dày của [text decoration](/docs/text-decoration-line) của một phần tử:
