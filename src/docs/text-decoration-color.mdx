...
export const description = "Các tiện ích để kiểm soát màu sắc của text decoration.";
...
Sử dụng các tiện ích như `decoration-sky-500` và `decoration-pink-500` để thay đổi màu [text decoration](/docs/text-decoration-line) của một phần tử:
...
Sử dụng modifier độ mờ màu để kiểm soát độ mờ màu text decoration của một phần tử:
