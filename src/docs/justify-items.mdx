...
export const description = "Các utility để kiểm soát cách các grid item được căn chỉnh dọc theo trục inline của chúng.";
...
Sử dụng utility `justify-items-start` để căn chỉnh các grid item về phía bắt đầu của trục inline của chúng:
...
Sử dụng các utility `justify-items-end` hoặc `justify-items-end-safe` để căn chỉnh các grid item về phía cuối của trục inline của chúng:
...
Khi không có đủ không gian, utility `justify-items-end-safe` sẽ căn chỉnh các item về phía bắt đầu của container thay vì phía cuối.

### Center

Sử dụng các utility `justify-items-center` hoặc `justify-items-center-safe` để căn chỉnh các grid item về phía cuối của trục inline của chúng:
...
Khi không có đủ không gian, utility `justify-items-center-safe` sẽ căn chỉnh các item về phía bắt đầu của container thay vì giữa.

### Stretch

Sử dụng utility `justify-items-stretch` để kéo giãn các item dọc theo trục inline của chúng:
