...
export const description = "Các tiện ích để kiểm soát cách hình nền hoạt động khi cuộn.";
...
Sử dụng tiện ích `bg-fixed` để cố định hình nền so với viewport:
...
Sử dụng tiện ích `bg-local` để cuộn hình nền cùng với container và viewport:
...
Sử dụng tiện ích `bg-scroll` để cuộn hình nền cùng với viewport, nhưng không cuộn cùng với container:
