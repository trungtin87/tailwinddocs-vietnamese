...
export const description = "Các tiện ích để chỉ định các hàng trong bố cục grid.";
...
### Chỉ định các hàng grid

Sử dụng các tiện ích `grid-rows-<number>` như `grid-rows-2` và `grid-rows-4` để tạo các grid với _n_ hàng có kích thước bằng nhau:
...
Sử dụng tiện ích `grid-rows-subgrid` để áp dụng các row track được định nghĩa bởi phần tử cha của item:
