...
export const description = "Các tiện ích để kiểm soát khoảng cách giữa các đường viền bảng.";
...
Sử dụng các tiện ích `border-spacing-<number>` như `border-spacing-2` và `border-spacing-x-3` để kiểm soát khoảng cách giữa các đường viền của các ô bảng với [separate borders](/docs/border-collapse#separating-table-borders):
