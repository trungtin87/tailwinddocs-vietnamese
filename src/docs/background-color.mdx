...
export const description = "Các tiện ích để kiểm soát màu nền của một phần tử.";
...
Sử dụng các tiện ích như `bg-white`, `bg-indigo-500` và `bg-transparent` để kiểm soát màu nền của một phần tử:
...
Sử dụng modifier độ mờ màu để kiểm soát độ mờ của màu nền của một phần tử:
