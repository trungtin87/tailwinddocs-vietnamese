...
export const description = "Các tiện ích để áp dụng bộ lọc backdrop blur cho một phần tử.";
...
Sử dụng các tiện ích như `backdrop-blur-sm` và `backdrop-blur-lg` để kiểm soát độ mờ backdrop của một phần tử:
