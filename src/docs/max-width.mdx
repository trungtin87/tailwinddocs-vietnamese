...
export const description = "Các utility để thiết lập chiều rộng tối đa của một phần tử.";
...
Sử dụng `max-w-full` hoặc các utility `max-w-<fraction>` như `max-w-1/2` và `max-w-2/5` để thiết lập chiều rộng tối đa dựa trên phần trăm cho một phần tử:
...
Sử dụng các utility như `max-w-sm` và `max-w-xl` để thiết lập chiều rộng tối đa cố định cho một phần tử dựa trên thang container:
