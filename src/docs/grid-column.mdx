...
export const description = "Các tiện ích để kiểm soát cách các phần tử được định kích thước và đặt trên các cột grid.";
...
Sử dụng các tiện ích `col-span-<number>` như `col-span-2` và `col-span-4` để làm cho một phần tử trải rộng trên _n_ cột:
...
Sử dụng các tiện ích `col-start-<number>` hoặc `col-end-<number>` như `col-start-2` và `col-end-3` để làm cho một phần tử bắt đầu hoặc kết thúc tại dòng grid thứ _n_:
