...
export const description = "Các utility để thiết lập chiều rộng của một phần tử.";
...
Sử dụng các utility `w-<number>` như `w-24` và `w-64` để thiết lập chiều rộng cố định cho một phần tử dựa trên thang khoảng cách:
...
Sử dụng `w-full` hoặc các utility `w-<fraction>` như `w-1/2` và `w-2/5` để thiết lập chiều rộng dựa trên phần trăm cho một phần tử:
...
Sử dụng các utility như `w-sm` và `w-xl` để thiết lập chiều rộng cố định cho một phần tử dựa trên thang container:
...
Sử dụng utility `w-screen` để làm cho một phần tử trải dài toàn bộ chiều rộng của viewport:
...
Ngoài ra, bạn có thể khớp chiều rộng của viewport lớn, nhỏ hoặc động bằng cách sử dụng các utility `w-lvw`, `w-svw`, và `w-dvw`.

### Đặt lại chiều rộng

Sử dụng utility `w-auto` để xóa chiều rộng đã gán của một phần tử trong một điều kiện cụ thể, như tại một breakpoint cụ thể:
