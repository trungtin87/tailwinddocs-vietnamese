...
export const description = "Các tiện ích để kiểm soát kiểu đường viền của một phần tử.";
...
Sử dụng các tiện ích như `border-solid` và `border-dotted` để kiểm soát kiểu đường viền của một phần tử:
...
Sử dụng tiện ích `border-none` để loại bỏ đường viền hiện có khỏi một phần tử:
...
Điều này thường được sử dụng nhất để loại bỏ kiểu đường viền đã được áp dụng ở điểm ngắt nhỏ hơn.

### Đặt kiểu divider

Sử dụng các tiện ích như `divide-dashed` và `divide-dotted` để kiểm soát kiểu đường viền giữa các phần tử con:
