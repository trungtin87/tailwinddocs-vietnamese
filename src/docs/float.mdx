...
### Float phần tử sang phải

Sử dụng tiện ích `float-right` để float một phần tử sang bên phải của container:
...
### Float phần tử sang trái

Sử dụng tiện ích `float-left` để float một phần tử sang bên trái của container:
...
Sử dụng các tiện ích `float-start` và `float-end`, sử dụng [logical properties](https://developer.mozilla.org/en-US/docs/Web/CSS/CSS_Logical_Properties/Basic_concepts) để ánh xạ sang bên trái hoặc bên phải dựa trên hướng văn bản:
...
Sử dụng tiện ích `float-none` để reset bất kỳ float nào được áp dụng cho một phần tử:
