...
export const description = "Các tiện ích để chỉ định các cột trong bố cục grid.";
...
### Chỉ định các cột grid

Sử dụng các tiện ích `grid-cols-<number>` như `grid-cols-2` và `grid-cols-4` để tạo các grid với _n_ cột có kích thước bằng nhau:
...
Sử dụng tiện ích `grid-cols-subgrid` để áp dụng các column track được xác định bởi cha của mục:
