...
export const description = "Các tiện ích để kiểm soát kiểu outline của một phần tử.";
...
Sử dụng các tiện ích như `outline-solid` và `outline-dashed` để đặt kiểu outline của một phần tử:
