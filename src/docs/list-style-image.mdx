import { ApiTable } from "@/components/api-table.tsx";
import { Example } from "@/components/example.tsx";
import { Figure } from "@/components/figure.tsx";
import { ResponsiveDesign } from "@/components/content.tsx";

export const title = "list-style-image";
export const description = "Các tiện ích để kiểm soát hình ảnh dấu đầu dòng cho các mục trong danh sách.";

<ApiTable
  rows={[
    ["list-image-[<value>]", "list-style-image: <value>;"],
    ["list-image-(<custom-property>)", "list-style-image: var(<custom-property>);"],
    ["list-image-none", "list-style-image: none;"],
  ]}
/>

## Ví dụ

### Ví dụ cơ bản

Sử dụng cú pháp `list-image-[<value>]` để kiểm soát hình ảnh dấu đầu dòng cho các mục trong danh sách:

<Figure>

<Example padding={false}>
  {
    <div className="px-4 sm:px-0">
      <div className="mx-auto max-w-sm p-8 text-gray-700 sm:text-base sm:leading-7 dark:text-gray-400">
        <ul className="list-image-[url('data:image/svg+xml;base64,')] pl-4 text-gray-900 dark:text-gray-200">
          <li className="pl-2">5 cups chopped Porcini mushrooms</li>
          <li className="pl-2">1/2 cup of olive oil</li>
          <li className="pl-2">3lb of celery</li>
        </ul>
      </div>
    </div>
  }
</Example>

```html
<!-- [!code classes:list-image-[url(/img/checkmark.png)]] -->
<ul class="list-image-[url(/img/checkmark.png)]">
  <li>5 cups chopped Porcini mushrooms</li>
  <!-- ... -->
</ul>
```

</Figure>

### Sử dụng biến CSS

Sử dụng cú pháp <code>list-image-{'(<custom-property>)'}</code> để kiểm soát hình ảnh dấu đầu dòng cho các mục trong danh sách sử dụng biến CSS:

```html
<!-- [!code classes:list-image-(--my-list-image)] -->
<ul class="list-image-(--my-list-image)">
  <!-- ... -->
</ul>
```

Đây chỉ là viết tắt cho <code>list-image-{'[var(<custom-property>)]'}</code> tự động thêm hàm `var()` cho bạn.

### Loại bỏ hình ảnh dấu đầu dòng

Sử dụng tiện ích `list-image-none` để loại bỏ hình ảnh dấu đầu dòng hiện có khỏi các mục trong danh sách:

```html
<!-- [!code classes:list-image-none] -->
<ul class="list-image-none">
  <!-- ... -->
</ul>
```

### Responsive design

<ResponsiveDesign
  property="list-style-image"
  defaultClass="list-image-none"
  featuredClass="list-image-[url(/img/checkmark.png)]"
/>
