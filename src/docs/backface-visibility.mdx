...
export const description = "Các tiện ích để kiểm soát liệu mặt sau của một phần tử có hiển thị hay không.";
...
Sử dụng tiện ích `backface-visible` để hiển thị mặt sau của một phần tử, như một khối lập phương, ngay cả khi nó được xoay ra khỏi tầm nhìn:
