...
export const description = "Các tiện ích để đặt chiều cao tối đa của một phần tử.";
...
Sử dụng các tiện ích `max-h-<number>` như `max-h-24` và `max-h-64` để đặt một phần tử có chiều cao tối đa cố định dựa trên spacing scale:
...
Sử dụng các tiện ích `max-h-full` hoặc `max-h-<fraction>` như `max-h-1/2` và `max-h-2/5` để cung cấp cho một phần tử chiều cao tối đa dựa trên phần trăm:
