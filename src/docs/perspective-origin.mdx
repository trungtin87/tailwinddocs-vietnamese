...
export const description = "Các tiện ích để kiểm soát perspective origin của một phần tử khi được đặt trong không gian 3D.";
...
Sử dụng các tiện ích như `perspective-origin-top` và `perspective-origin-bottom-left` để kiểm soát vị trí điểm tụ của phối cảnh:
