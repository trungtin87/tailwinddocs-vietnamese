...
export const description = "Các tiện ích để áp dụng bộ lọc backdrop opacity cho một phần tử.";
...
Sử dụng các tiện ích như `backdrop-opacity-50` và `backdrop-opacity-75` để kiểm soát độ mờ của tất cả các bộ lọc backdrop được áp dụng cho một phần tử:
