...
export const description = "Các tiện ích để kiểm soát việc bao bọc nội dung xung quanh một phần tử.";
...
Sử dụng tiện ích `clear-left` để đặt một phần tử bên dưới các phần tử float bên trái trước đó:
...
Sử dụng tiện ích `clear-right` để đặt một phần tử bên dưới các phần tử float bên phải trước đó:
...
Sử dụng tiện ích `clear-both` để đặt một phần tử bên dưới tất cả các phần tử float trước đó:
...
Sử dụng các tiện ích `clear-start` và `clear-end`, sử dụng [logical properties](https://developer.mozilla.org/en-US/docs/Web/CSS/CSS_Logical_Properties/Basic_concepts) để ánh xạ sang bên trái hoặc bên phải dựa trên hướng văn bản:
...
Sử dụng tiện ích `clear-none` để reset bất kỳ clear nào được áp dụng cho một phần tử:
