...
export const description = "Các tiện ích để đặt chiều cao tối thiểu của một phần tử.";
...
Sử dụng các tiện ích `min-h-<number>` như `min-h-24` và `min-h-64` để đặt một phần tử có chiều cao tối thiểu cố định dựa trên spacing scale:
...
Sử dụng các tiện ích `min-h-full` hoặc `min-h-<fraction>` như `min-h-1/2`, và `min-h-2/5` để cung cấp cho một phần tử chiều cao tối thiểu dựa trên phần trăm:
