...
export const description = "Các tiện ích để kiểm soát độ mờ của một phần tử.";
...
Sử dụng các tiện ích `opacity-<number>` như `opacity-25` và `opacity-100` để đặt độ mờ của một phần tử:
