...
export const description = "Các tiện ích để kiểm soát hộp giới hạn của mask của một phần tử.";
...
Sử dụng các tiện ích như `mask-clip-border`, `mask-clip-padding`, và `mask-clip-content` để kiểm soát hộp giới hạn của mask của một phần tử:
