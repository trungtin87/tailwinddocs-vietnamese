...
export const description = "Các tiện ích để kiểm soát easing của CSS transition.";
...
Sử dụng các tiện ích như `ease-in` và `ease-out` để kiểm soát đường cong easing của transition của một phần tử:
