...
export const description =
  "Các tiện ích để kiểm soát cách một mục riêng lẻ được căn chỉnh (justify) và căn hàng (align) cùng một lúc.";
...
Sử dụng `place-self-auto` để căn chỉnh một mục dựa trên giá trị thuộc tính `place-items` của container:
...
Sử dụng `place-self-start` để căn chỉnh một mục về phía đầu trên cả hai trục:
...
Sử dụng `place-self-center` để căn chỉnh một mục ở giữa trên cả hai trục:
...
Sử dụng `place-self-end` để căn chỉnh một mục về phía cuối trên cả hai trục:
...
Sử dụng `place-self-stretch` để kéo dãn một mục trên cả hai trục:
