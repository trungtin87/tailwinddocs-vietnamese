...
export const description = "Các tiện ích để kiểm soát cách các mục flex co lại.";
...
### Cho phép các mục flex co lại

Sử dụng `shrink` để cho phép một mục flex co lại nếu cần:
...
Sử dụng `shrink-0` để ngăn cản một mục flex co lại:
