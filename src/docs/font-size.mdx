...
export const description = "Các tiện ích để kiểm soát kích thước font chữ của một phần tử.";
...
Sử dụng các tiện ích như `text-sm` và `text-lg` để đặt kích thước font chữ của một phần tử:
...
Sử dụng các tiện ích như `text-sm/6` và `text-lg/7` để đặt kích thước font chữ và line-height của một phần tử cùng một lúc:
...
Bạn cũng có thể cung cấp các giá trị mặc định cho `line-height`, `letter-spacing`, và `font-weight` cho một kích thước font chữ:
