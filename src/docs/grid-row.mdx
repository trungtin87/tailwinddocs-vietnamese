...
export const description = "Các tiện ích để kiểm soát cách các phần tử được định kích thước và đặt trên các hàng grid.";
...
### Kéo dài qua các hàng

Sử dụng các tiện ích `row-span-<number>` như `row-span-2` và `row-span-4` để làm cho một phần tử kéo dài qua _n_ hàng:
...
Sử dụng các tiện ích `row-start-<number>` hoặc `row-end-<number>` như `row-start-2` và `row-end-3` để làm cho một phần tử bắt đầu hoặc kết thúc tại dòng grid thứ _n_:
...
Các tiện ích này cũng có thể được kết hợp với các tiện ích `row-span-<number>` để kéo dài qua một số lượng hàng cụ thể.
