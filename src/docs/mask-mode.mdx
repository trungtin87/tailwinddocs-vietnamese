...
export const description = "Các tiện ích để kiểm soát chế độ mask của một phần tử.";
...
Sử dụng các tiện ích `mask-alpha`, `mask-luminance` và `mask-match` để kiểm soát chế độ mask của một phần tử:
...
Khi sử dụng `mask-luminance`, giá trị độ sáng của mask xác định khả năng hiển thị, vì vậy việc sử dụng các màu grayscale sẽ tạo ra kết quả dễ đoán nhất. Với `mask-alpha`, độ mờ của mask xác định khả năng hiển thị của phần tử được mask.
