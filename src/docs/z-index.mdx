...
export const description = "Các tiện ích để kiểm soát thứ tự xếp chồng của một phần tử.";
...
Sử dụng các tiện ích `z-<number>` như `z-10` và `z-50` để kiểm soát thứ tự xếp chồng (hoặc định vị ba chiều) của một phần tử, bất kể thứ tự nó được hiển thị:
