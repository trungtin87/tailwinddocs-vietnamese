...
export const description = "Các utility để kiểm soát border radius của một phần tử.";
...
Sử dụng các utility như `rounded-sm` và `rounded-md` để áp dụng các kích thước border radius khác nhau cho một phần tử:
...
Sử dụng các utility như `rounded-t-md` và `rounded-r-lg` để chỉ bo tròn một cạnh của một phần tử:
...
Sử dụng các utility như `rounded-tr-md` và `rounded-tl-lg` để chỉ bo tròn một góc của một phần tử:
...
Sử dụng các utility như `rounded-s-md` và `rounded-se-xl` để thiết lập border radius bằng cách sử dụng [các thuộc tính logic](https://developer.mozilla.org/en-US/docs/Web/CSS/CSS_Logical_Properties/Basic_concepts), ánh xạ tới các góc thích hợp dựa trên hướng văn bản:
...
Để kiểm soát nhiều hơn, bạn cũng có thể sử dụng [các modifier LTR và RTL](/docs/hover-focus-and-other-states#rtl-support) để áp dụng có điều kiện các style cụ thể tùy thuộc vào hướng văn bản hiện tại.
